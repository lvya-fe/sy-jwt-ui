...
export default {
    ...
    methods:{
        ...
        loadData(){
          var _self = this;
          this.$axios.post( process.env.API_ROOT+"app/stu/v1/inquiryList",
            qs.stringify({
              uid:_self.$route.params.uid
            })
          ).then(function(res){
            _self.conData=[];
            res.data.listOpinion.forEach(function(item) {
                _self.conData.push(
                    {
                        stu:item.stu,
                        lookY:false,
...
<style scoped>
.top-back {text-align: center;font-size: 36px;color: #444;padding:20px 20px 10px 20px;background-color: #fff;position: fixed;top:0;left:0;right:0;z-index: 100;}
...
.bottom-div div:nth-child(1){float: left;width: 580px;color: #444;font-size: 24px;margin-left:110px;padding-top:2px;}
