<template>
  <div>
    ...
    <div class="class-con">
      <div v-for="(item,index) in conData" :key="index" @click="select(index)" :style="index==0?'color:#2ed184':''" :class="indexCycle==index?'selected':''">{{ item.stime + ' ~ '}} {{ (jointimetype==0&&cycle==0&&(formZt=='X'||formZt=='N'))?'进行中':item.etime  }}</div>
    </div>
  </div>
</template>
