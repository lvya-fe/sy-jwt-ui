                ...
                <dir class="stuListPar">
                    <div class="ripple" v-for="(item,index) in conData" :key="index" @click="link(item.task.id,item.stu.id,item.task.joinstarttime,item.task.joinendtime,item.stu.name)">
                        ...
                        <div class="ne-div">
                            ...
                            <p>
                                {{item.taskStateName}}<span>{{item.updateTimeStr}}</span>
                                <!-- {{ 
                                    item.taskStateName=='已完成'?'完成时间 :':
                                (item.taskStateName=='未完成'||item.taskStateName=='进行中')&&item.task.jointimetype==0?'不限制':
                                (item.taskStateName=='未完成'||item.taskStateName=='进行中')&&item.task.jointimetype==2?'剩余时间 :':
                                item.taskStateName=='待审核'?'提交时间 :':
                                item.taskStateName=='未通过'?'审核时间 :':
                                item.taskStateName=='已经结束'&&item.task.jointimetype==2?'结束时间 :':
                                item.taskStateName=='尚未开始'&&item.task.jointimetype==2?'开启时间:':'' }}
                                <span>{{ 
                                    (item.taskStateName=='尚未开始'&&item.task.jointimetype==2&&item.task.limittimes.length==0)?item.task.joinstarttime:
                                    (item.taskStateName=='尚未开始'&&item.task.jointimetype==2&&item.task.limittimes.length>0)?formatDuringNextDay(item.task.limittimes):
                                    item.taskStateName=='已完成'||item.taskStateName=='待审核'||item.taskStateName=='未通过'? item.task.joinStartTimeShow:
                                    item.taskStateName=='已经结束'&&item.task.jointimetype==2?item.task.joinendtime:
                                    (item.taskStateName=='未完成'||item.taskStateName=='进行中')&&item.task.jointimetype==2&&item.task.limittimes.length==0?formatDuring(item.task.joinendtime):
                                    (item.taskStateName=='未完成'||item.taskStateName=='进行中')&&item.task.jointimetype==2&&item.task.limittimes.length>0?formatDuringToday(item.task.limittimes):''
                                    }}</span>  -->
                                </p>
...
export default {
    ...
    methods:{
        ...
        query(){
            console.log(this.id)
            this.$axios.get( process.env.API_ROOT+"app/stu/v1/showStuTaskList-test",
                qs.stringify({
                    uid:this.uid,
