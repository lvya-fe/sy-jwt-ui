    ...
    <div>
        ...
        <div v-if="(formZt=='X'||formZt=='N')&&zt!=0" class="taskdetails_content" v-show="show3">
            ...
            <template v-for="item in itmes">
                ...
                <!-- 图片上传 -->
                <p v-if="item.type==6&&item.citeDataType!=1" class="pic_p"><strong :style="item.notnull=='Y'?'color:#ff0000;vertical-align: middle;':''"> {{ (item.notnull=='Y')?'*':'•' }}</strong>{{ item.name }}</p>
                <uploadImg v-if="item.type==6&&item.citeDataType!=1" v-bind:imgs.sync="item.val" v-bind:uid.sync="uid" v-bind:count.sync="count"></uploadImg>

                <div class="referenceDiv" v-if="item.citeDataType==1&&item.type==6">
                    <span>"{{ item.name }}"</span>
                    <div class="referChildDiv" v-for="(itemRefer,index) in item.listCiteData">
                        <p><span>{{ itemRefer.name }}</span> <span>{{ itemRefer.orgNames }}</span></p>
        ...
        <showhistime v-if="showtime" v-bind:stime="strTime" v-bind:etime="endTime" v-bind:uid="uid" v-bind:id="id" v-bind:stuid="stuid" @closeSelect1="closeSelect1" v-bind:indexCycle1="indexCycle1" @selectCycle1="selectCycle1" v-bind:formZt="formZt"></showhistime>
...
import showhistime from '@/page/tea/SubmissionTime'
...
export default {
    ...
    methods:{
        ...
        selectCycle1(id,v){
            ...
            this.$axios.post( process.env.API_ROOT+"app/tea/task/"+urlName,
                qs.stringify(stt)
            ).then(function(res){
                _self.show3 =true;
        ...
        loadData(){
          ...
          console.log(stt,'看sttttttt')
          this.$axios.post( process.env.API_ROOT+"app/tea/task/"+urlName,
            qs.stringify(stt)
          ).then(function(res){
            _self.itmesRe=res.data.itmes
            _self.task = res.data.task;
            _self.itmes = res.data.itmes;
            _self.zt = res.data.task.status;
        ...
        obtain(element,number){
            ...
            if (!myreg.test(number)) {
                ...
                this.$axios.post( process.env.API_ROOT+"app/tea/task/sendCode",
                    qs.stringify({
                        stuId:_self.stuid,
                        uid:_self.uid,
                        phone:number
