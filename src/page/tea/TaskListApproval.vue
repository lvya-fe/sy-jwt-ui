    ...
    <div v-show="isshow">
        <div class="top-back">
            ...
            <group class="right-btn" v-show="taskinfo.jointimetype==1">
                <x-switch title="是否开启" v-model="taskinfo.islock" @on-change="changeTask"></x-switch>
            </group>
        </div>
        <div class="tab-con">
                ...
                <group>
                  <template  v-for="stu in write">
                    ...
                    <cell-box class="con-child"  @click.native="to(stu.id)" >
                        <p>{{stu.type=='teaToStu'||stu.type=='stu' ||stu.type=='stuToStu'?stu.stuName:stu.teaName}}<br><span>{{stu.type=='tea'?stu.teaOrgName.split(',')[0]: ['stu','stuToStu'].includes(stu.type) ? stu.stuOrgName:stu.type=='teaToStu'?stu.stuOrgName+' 填写人：'+stu.teaName:''}}</span></p>
            ...
            <div class="tab-swiper vux-center" v-show="index == 3">
              ...
              <!--</scroller>-->
                <button type="button" class="pl-btn" @click="sendNotice(nowrite[0].type)" ref="up_btn" :disabled="selectStuId.length==0">批量通知</button>
            </div>

        </div>
    </div>
    <showcycle v-show="!isshow"  v-bind:indexCycle="indexCycle" v-bind:conData="taskCycles" @closeSelect="closeSelect" @selectCycle="selectCycle"></showcycle>
  </div>
</template>
    ...
    methods:{
        ...
        selectCycle(v){
          ...
          //  _self.tarkName[0].num=   res.data.taskWriteStuCount;
            _self.tarkName[3].num=   res.data.taskNoWriteStuCount;
          }).catch(function(err){
            _self.errorUtil(err);
          })
          this.index=0;
        ...
        sendNotice(type){
          if(type=='tea'||type=='teaToStu'){
            ...
            var json = {sceneid:this.id, relateid:this.id,relatetype:'4',sendids:stus,ptype:'3'};
            var jsonsrt =  JSON.stringify(json);
            this.$cookie.set('notified', jsonsrt, 1);
            this.$router.push({path: '/notified/'+this.uid+'/'+this.id});

            // this.$router.push({path: '/notified/'+this.uid,name:'notified',params:{sceneid:this.id,relateid:this.taskid,relatetype:'4',sendids:stus,ptype:'3'}});
          }
        },
        loadData(){
          var _self = this;
          this.$axios.post( process.env.API_ROOT+"app/tea/task/index",
            ...
            _self.taskCycles = res.data.taskCycles;

            _self.indexCycle = res.data.taskCycles.length-1;

            _self.tarkName[0].num=   0;
            _self.tarkName[1].num=   0;
            _self.tarkName[2].num=   0;
            _self.tarkName[3].num=   res.data.taskNoWriteStuCount;
      ...
      QueryWrite(){
          ...
          this.$axios.post( process.env.API_ROOT+"app/tea/task/queryWriteForms",
            ...
            console.log(res)
             _self.write = res.data;

             var c0 = 0;
             var c1 = 0;
             var c2 = 0;
             _self.write.forEach(function (task) {
                  if(task.state=='O'){
                    c0 = c0+1;
                  }
                  if(task.state=='N'){
            ...
            _self.tarkName[0].num=   c0;
            _self.tarkName[1].num=   c1;
            _self.tarkName[2].num=   c2;
...
.tab-con .tab-swiper > div .vux-no-group-title{margin-top: 0;}
...
.top-back .img-2{position: absolute;width: 38px;height: 38px;left: 120px;top: 25px}
.top-back .img-2 img{width: 42px;height: 42px;}
.right-btn{position: absolute;right: 25px;top:0;}
...
.checkBox {width: 25px;height: 25px;background-color: #fff;-webkit-appearance:none;border: 1px solid #c9c9c9;/*no*/border-radius: 5px/*bo*/;outline: none;padding: 2px;}
...
.task-con-div{position: absolute;bottom:100px;top:190px;left:0;right:0;overflow: hidden;}
