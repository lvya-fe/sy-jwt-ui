<template>
  ...
  <div class="whole">
    <div class="top-div">
      <div class="title">
        <img class="left-img" src="../../assets/img/back_left_green.png" alt="" @click="qx"> 通知人员
      ...
      <div class="tab-div">
        <div :class="(type==1)?'selected':''" @click="tag(1)">过程任务</div>
        <div :class="(type==2)?'selected':''" @click="tag(2)">部门班级</div>
        <div :class="(type==3)?'selected':''" @click="tag(3)">学生</div>
      </div>
    </div>


    <div class="con-area" v-show="s1">
      <div class="con-area-child">
        <input class="checkBox" type="checkbox" @click="chooseScene1All" ref="all1">
        <span>全选</span>
      </div>
      <div class="con-area-child" v-for="(scene,index) in listScene"  >
        <input class="checkBox" type="checkbox"  @change="chooseScene1(index)"  v-model="scene.check">
        <span @click="sceneIn(scene,index)">{{scene.title}}</span>

        <strong class="select-dian" v-show="scene.dian" >•</strong>
        <img class="right-img" src="../../assets/img/you.png" alt=""
        @click="sceneIn(scene,index)">
      </div>
    </div>


    <div class="con-area" v-show="s2">
      <div class="child-title">
        <span @click="s2=false;s1=true">{{scenename}} /</span>
        <span>{{taskname}}</span>
      </div>
      <div class="con-area-child">
        <input class="checkBox" type="checkbox" @click="chooseScene2All" ref="all2">
        <span>全选</span>
      </div>
      <div class="con-area-child"  v-for="(task,index) in scenelist">
        <input class="checkBox" @change="chooseScene2(index)" type="checkbox" checked="true"  v-model="task.check">
        <span>{{task.title}}</span>
      </div>
    </div>


    <div class="con-area" v-show="s3">
      <div class="child-title">
        <span >查询{{query}} </span>
      </div>
      <div class="con-area-child"  v-for="(task,index) in queryscenelist">
        <input class="checkBox" @change="chooseScene2(index)" type="checkbox" checked="true"  v-model="task.check">
        <span>{{task.title}}</span>
      </div>
    </div>
    ...
    <div class="bottom-title" @click.prevent.stop="tallBtn($event)" v-show="s1">
      已选：{{selecttask}}
    </div>
    ...
    <div class="con-area" v-show="o2">
      <div class="child-title">
        <span >查询{{query}} </span>
      </div>
      <div class="con-area-child"  v-for="(org,index) in queryorglist">
        <input class="checkBox" @change="chooseOrg(org)" type="checkbox" checked="true"  v-model="org.check">
        <span>{{org.title}}</span>
      </div>
    </div>
    <button type="button" class="bottom-title-btn" v-show="o2" @click="queryOut">返回</button>
    ...
    <div class="con-area" v-show="stu1">
      <div class="con-area-child" v-for="(org,index) in orgslsit">
    ...
    <div class="con-area" v-show="stu2">
       <div class="child-title">
        ...
        <div class="con-area-child" v-for="(stu, key, index) in stumap[orgid]"
             v-bind:key="stu.id">
     ...
     <div class="con-area" v-show="stu3">
        ...
        <div class="con-area-child"  v-for="(stu,index) in queryStulist">
          <input class="checkBox" @change="chooseStu(stu)" type="checkbox" checked="true"  v-model="stu.check">
          <span>{{stu.name}}</span>
        </div>
    </div>
    ...
    <button type="button" class="bottom-title-btn" v-show="o3" @click="orgOut">返回上一级</button>
    <button type="button" class="bottom-title-btn" v-show="s2" @click="s2=false;s1=true">返回上一级</button>
  </div>
...
export default {
  ...
  computed: {
    ...
    queryscenelist:function () {
       ...
       this.listScene.forEach(function(item) {
            item.taskbeans.forEach(function(item2) {
              if(item2.title.indexOf(self_.query)!=-1){
                querytask.push(item2);
              }
            });
        });
        return querytask;
    ...
    queryorglist:function(){
      ...
      var queryorg = [];
      this.listOrg.forEach(function(item) {
          if(item.title.indexOf(self_.query)!=-1){
            queryorg.push(item);
          }
        });
        return queryorg;
    ...
    scenelist:function () {
        ...
        var flg = true;
        var flg2 = false;
        this.listScene[this.s1index].taskbeans.forEach(function(item2) {
          if(!item2.check){
            flg = false;
          }else{
            flg2 = true;
        ...
        this.$refs.all2.checked = flg;
        this.listScene[this.s1index].check = flg;
        this.listScene[this.s1index].dian = flg2;
        return this.listScene[this.s1index].taskbeans
    },
    stulist:function() {
        var self_ = this;
  ...
  methods:{
    qx(){
        var self_ = this;
        // self_.listScene.forEach(function(item) {
        //   item.check = false;
        //   item.dian = false;
        //   item.taskbeans.forEach(function(item2) {
        //     item2.check = false;
    ...
    qd(){
      ...
      if(self_.type=="1"){
        self_.listScene.forEach(function(item) {
          item.taskbeans.forEach(function(item2) {
            if(item2.check){
              rs.push({type:self_.type,id:item2.id,name:item2.title});
            }
          });
        });
      }
    ...
    chooseScene2All(event){
      var flg = event.currentTarget.checked;
      this.listScene[this.s1index].taskbeans.forEach(function(item) {
         item.check = flg;
      });

      this.listScene[this.s1index].check = flg;
      this.listScene[this.s1index].dian = flg;
    },
    ...
    chooseScene1(index){
        var flg =this.listScene[index].check;
        this.listScene[index].taskbeans.forEach(function(item2) {
          item2.check=flg;
        });
      ...
      this.$refs.all1.checked =flg_;


    },
    chooseScene2(index){
      this.s2index= index;
      var flg =this.listScene[ this.s1index].taskbeans[index].check;
      if(!flg) {
        this.listScene[this.s1index].check = flg;
      }
    ...
    querystu(){
        ...
        this.$axios.post( process.env.API_ROOT+"app/tea/common/select/selectStu",
          ...
          _self.queryStulist = res.data.listStu;
          // res.listStu.forEach(function(item) {
          //   _self.stus.push(item);
          // });
          // _self.stumap[orgid] = res.listStu;
        }).catch(function(err){
          _self.errorUtil(err);
        })
    },
    loadData(){
      var _self = this;
      this.$axios.post( process.env.API_ROOT+"app/tea/common/select/selecttask",
        qs.stringify({
          uid:_self.$route.params.uid,
          sceneid:_self.sceneid
        })
      ).then(function(res){
        _self.listScene = res.data.listScene;
      }).catch(function(err){
        _self.errorUtil(err);
      })
    },
    ...
    chooseStu(stu){
        ...
        if(this.stumap[stu2.orgid]!=undefined){
          ...
          if(!flg){
            this.stumap[stu2.orgid].push(stu2);
          }
        }else{
          this.stumap[stu2.orgid] = new Array();
          this.stumap[stu2.orgid].push(stu2);
        }
    ...
    stuIn(org){
      ...
      if(this.stumap[orgid]!=undefined){
           this.stumap[orgid].forEach(function(item) {
              if(item.check){
              index_ = index_+item.id+",";
              }
          });
      }
      ...
      this.$axios.post( process.env.API_ROOT+"app/tea/common/select/selectStu",
        ...
        // 非空的情况将已经选择的值传递下去
          ...
          res.data.listStu.forEach(function(item) {
                var flg = true;
                _self.stus.forEach(function(item2) {
                if(item2.id==item.id){
                  item2 = item;
                  flg = false;
                }
...
<style scoped>
  *{box-sizing: border-box}
  ...
  .title .button{position: absolute;right: 20px;top: 20px;;width: 138px;height: 48px;border: 1px solid #fff;/*no*/color: #01c269;outline: none;background: #fff;font-size: 36px;border-radius: 5px;/*no*/}
  ...
  .con-area-child span {font-size: 28px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;margin-left: 50px;width:100%;}
  .con-area-child-ts span {font-size: 28px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;margin-left: 30px;}
  ...
  .child-title{font-size: 28px;padding: 30px ;color: #01c269;border-bottom: 1px solid #e7e6e6;/*no*/}
